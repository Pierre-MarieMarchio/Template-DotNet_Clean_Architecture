...
namespace AppTemplate.Application.UnitTests.Features.TodoLists.Concurrency;
...
public sealed class VersionPreconditionTests
{
    ...
    public async Task AnOperationDecidedAgainstAnOlderVersion_IsRefused(string operation)
    {
        ...
        result.Error!.Type.ShouldBe(ErrorType.PreconditionFailed, operation);
        result.Error.Code.ShouldBe("precondition.failed", operation);
    }
    ...
    private static async Task<Result> Widened(Task<Result<Guid>> execution) => await execution;
        ...
        VersionPrecondition? precondition)
    {
        ...
        {
            _rename => new RenameTodoListUseCase(
                    _repository,
                    _unitOfWork,
                    currentUser,
                    new RenameTodoListCommandValidator())
                .ExecuteAsync(new RenameTodoListCommand(list.Id, "Renamed", precondition), TestToken),

            _delete => new DeleteTodoListUseCase(_repository, _unitOfWork, currentUser)
                .ExecuteAsync(new DeleteTodoListCommand(list.Id, precondition), TestToken),

            _addItem => Widened(
                new AddTodoItemUseCase(
                        _repository,
                        _unitOfWork,
                        currentUser,
                        new AddTodoItemCommandValidator())
                    .ExecuteAsync(
                        new AddTodoItemCommand(list.Id, "A second item", null, null, precondition),
                        TestToken)),

            _completeItem => new CompleteTodoItemUseCase(
                    _repository,
                    _unitOfWork,
                    currentUser,
            ...
            _removeItem => new RemoveTodoItemUseCase(_repository, _unitOfWork, currentUser)
