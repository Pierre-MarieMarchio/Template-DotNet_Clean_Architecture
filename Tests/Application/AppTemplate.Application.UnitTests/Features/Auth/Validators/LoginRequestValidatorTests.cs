...
public sealed class LoginRequestValidatorTests
{
    ...
    public void AWellFormedRequest_IsAccepted() =>
        _validator.Validate(new LoginRequest("someone@example.com", "correct horse battery"))
    ...
    public void AnIncompleteRequest_IsRejected(string email, string password) =>
        _validator.Validate(new LoginRequest(email, password)).IsValid.ShouldBeFalse();
    ...
    public void AMalformedEmail_IsAcceptedBecauseLoginChecksPresenceOnly() =>
        _validator.Validate(new LoginRequest("not-an-email", "correct horse battery")).IsValid.ShouldBeTrue();
    ...
    public void AShortPassword_IsAcceptedBecauseAnExistingAccountMayPredateThePolicy() =>
        _validator.Validate(new LoginRequest("someone@example.com", "a")).IsValid.ShouldBeTrue();
}
