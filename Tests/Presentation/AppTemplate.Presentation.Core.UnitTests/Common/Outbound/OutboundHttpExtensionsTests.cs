...
namespace AppTemplate.Presentation.Core.UnitTests.Common.Outbound;

/// <summary>
/// The verb allow-list the policy wraps around the package's own idea of a transient failure, and
/// the budget it writes down.
/// <para>
/// <c>OutboundHttpTests</c> in the architecture project proves each host installs the policy and
/// that nothing constructs its own <c>HttpClient</c> around it; neither reads a value, so a
/// <c>PUT</c> becoming replayable — the thing the policy argues hardest for — would only be seen
/// here.
/// </para>
/// </summary>
public sealed class OutboundHttpExtensionsTests
{
    ...
    public void TheBudget_IsWrittenDownRatherThanInherited()
    {
        ...
        using (provider)
        {
            ...
            // budget changes when the package's does.
