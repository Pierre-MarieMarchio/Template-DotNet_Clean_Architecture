...
public sealed class TwoFactorChallengeTests
{
    ...
    public async Task RedeemAsync_AMalformedToken_IsRejected(string malformed)
    {
        ...
        redemption.Outcome.ShouldBe(TwoFactorRedemptionOutcome.InvalidChallenge);
    }
    ...
    public async Task RedeemAsync_ARightAuthenticatorCode_IsVerified()
    {
        ...
        redemption.Outcome.ShouldBe(TwoFactorRedemptionOutcome.Verified);
    ...
    public async Task RedeemAsync_ARightCode_ConsumesTheChallenge()
    {
        ...
        replay.Outcome.ShouldBe(TwoFactorRedemptionOutcome.InvalidChallenge);
    }
    ...
    public async Task RedeemAsync_ARightRecoveryCode_CannotBeReplayedOnANewChallenge()
    {
        ...
        replay.Outcome.ShouldBe(TwoFactorRedemptionOutcome.InvalidCode);
    }

    /// <summary>A mistyped code should not force the caller back through <c>/login</c>.</summary>
    [Fact]
    public async Task RedeemAsync_AWrongCode_LeavesTheChallengeLiveForARetry()
    {
        ...
        wrong.Outcome.ShouldBe(TwoFactorRedemptionOutcome.InvalidCode);

        var right = await challenges.RedeemAsync(issued.ChallengeToken, AuthenticatorCodes.CurrentCodeFor(sharedKey), TestToken);
        right.Outcome.ShouldBe(TwoFactorRedemptionOutcome.Verified);
    }
