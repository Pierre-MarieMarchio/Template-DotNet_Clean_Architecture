...
/// <para>
/// Deliberately not <c>ITwoFactorEnrollmentService</c>: that capability is reached by the account itself and
/// every operation on it is gated by a credential only the account holder has — see
/// <c>ITwoFactorEnrollmentService.DisableAsync</c>. This one exists for exactly the case that gate cannot
...
/// the only recourse left is <c>IAccountDeletionService</c>, which is a much larger hammer than the second
/// factor being stuck on.
/// </para>
/// </summary>
public interface ITwoFactorAdministrationService
{
    /// <summary>
    /// Turns two-factor sign-in off unconditionally — no password, no code, no proof of the sort
    /// <c>ITwoFactorEnrollmentService.DisableAsync</c> demands, because an administrator reaching this
