...
namespace AppTemplate.Infrastructure.Storage.Features.Files.Stores;
...
{
    /// <summary>
    /// Carries the SHA-256 the content must have, base64-encoded. Part of the signature, so a deposit
    ...
    /// </summary>
    /// <remarks>
    /// A value that is not 64 hexadecimal characters is a programming error rather than a client's
    /// mistake — <c>Sha256Checksum</c> refuses one on the way in — so it throws rather than being
    /// reported, and it throws before anything is signed.
    /// </remarks>
    private static string Base64DigestOf(string declaredSha256)
    {
        if (declaredSha256.Length != _sha256DigestLength * 2)
        {
            throw new ArgumentException(
                $"A SHA-256 is {_sha256DigestLength * 2} hexadecimal characters; this one is " +
                $"{declaredSha256.Length}.",
                nameof(declaredSha256));
        }
