...
/// <see cref="TodoListRequestBinder.Bind"/> directly rather than through a use case and a
/// mocked port.
/// </summary>
public sealed class TodoListRequestBinderTests
{
    ...
    {
        var result = TodoListRequestBinder.Bind(GetTodoListsQuery.Offset(page, 10));
    ...
    {
        var result = TodoListRequestBinder.Bind(GetTodoListsQuery.Offset(1, pageSize));
    ...
    {
        var result = TodoListRequestBinder.Bind(GetTodoListsQuery.Offset(1, _policy.MaxPageSize + 1));
    ...
    public void APageSizeInsideTheAllowedRange_IsAccepted(int pageSize) =>
        TodoListRequestBinder.Bind(GetTodoListsQuery.Offset(1, pageSize)).IsSuccess.ShouldBeTrue();

    [Fact]
    public void ThePageSizeCeiling_IsItselfAccepted() =>
        TodoListRequestBinder.Bind(GetTodoListsQuery.Offset(1, _policy.MaxPageSize)).IsSuccess.ShouldBeTrue();
    ...
    {
        var result = TodoListRequestBinder.Bind(
    ...
    public void ASingleTermSortInCursorMode_IsAccepted() =>
        TodoListRequestBinder.Bind(new GetTodoListsQuery("cursor", null, 10, null, "name", null, null, null))
    ...
    public void ATamperedCursorKey_IsAValidationFailure_NotAnUnhandledException()
    {
        ...
        result.IsFailure.ShouldBeTrue();
        result.Error!.Type.ShouldBe(AppTemplate.Application.Common.ErrorType.Validation);
        result.Error.Code.ShouldBe("cursor.invalid");
    }
