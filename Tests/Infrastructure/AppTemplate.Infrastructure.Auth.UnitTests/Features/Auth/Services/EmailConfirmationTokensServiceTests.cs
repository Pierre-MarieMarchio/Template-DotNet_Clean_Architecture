...
namespace AppTemplate.Infrastructure.Auth.UnitTests.Features.Auth.Services;
...
public sealed class EmailConfirmationTokensServiceTests
{
    ...
    /// <see cref="EmailConfirmationStatus.Confirmed"/> instead of
    /// <see cref="EmailConfirmationStatus.InvalidToken"/> on the replay.
    /// </summary>
    [Fact]
    public async Task ARedeemedToken_CannotBeReplayed()
    {
        ...
        first.ShouldBe(EmailConfirmationStatus.Confirmed);

        var replay = await tokens.RedeemAsync(_knownEmail, token, TestToken);
        replay.ShouldBe(EmailConfirmationStatus.InvalidToken);
    }
    ...
    public async Task AWrongToken_IsRefusedAndRotatesNothing()
    {
        ...
        outcome.ShouldBe(EmailConfirmationStatus.InvalidToken);
    ...
    public async Task AnUnknownAddress_IsRefused()
    {
        ...
        outcome.ShouldBe(EmailConfirmationStatus.NoSuchAccount);
    }
