    ...
    IAppUserDirectory directory) : IEmailChangeTokens
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return EmailChangeRequestOutcome.Issued(
            user.UserName ?? string.Empty,
            await userManager.GenerateChangeEmailTokenAsync(user, newEmail));
    }

    public async Task<EmailChangeConfirmationOutcome> RedeemAsync(
        Guid userId,
        string newEmail,
        string token,
        CancellationToken cancellationToken = default)
    {
        ...
        if (result.Succeeded)
        {
            return EmailChangeConfirmationOutcome.Changed;
        }
        ...
        return invalidToken
            ? EmailChangeConfirmationOutcome.InvalidToken
            : EmailChangeConfirmationOutcome.Rejected(string.Join(" ", result.Errors.Select(error => error.Description)));
    }
}
