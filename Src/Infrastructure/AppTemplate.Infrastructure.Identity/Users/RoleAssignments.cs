...
/// exception rather than a reported <see cref="RoleAssignmentChangeOutcome.Rejected"/>.
...
{
    public async Task<RoleAssignmentChange> AddRoleAsync(
        Guid userId,
        string role,
        CancellationToken cancellationToken = default)
    {
        ...
        if (user is null)
        {
            return RoleAssignmentChange.NoSuchAccount;
        }

        if (!await roleManager.RoleExistsAsync(role))
        {
            return RoleAssignmentChange.Rejected($"Role '{role}' does not exist.");
        }
        ...
        if (!result.Succeeded)
        {
            return RoleAssignmentChange.Rejected(string.Join(" ", result.Errors.Select(error => error.Description)));
        }
        ...
        return RoleAssignmentChange.Applied;
    }

    public async Task<RoleAssignmentChange> RemoveRoleAsync(
        Guid userId,
        string role,
        CancellationToken cancellationToken = default)
    {
